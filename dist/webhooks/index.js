...
async function verifyStripeSignatureFromText(opts) {
  const passOpts = {
    payload: encodeUtf8(opts.payload),
    header: opts.header,
    secret: opts.secret
  };
  if (opts.tolerance !== void 0) passOpts.tolerance = opts.tolerance;
  if (opts.now !== void 0) passOpts.now = opts.now;
...
export { createWebhookHandler, parseEvent, parseSignatureHeader, verifyStripeSignature, verifyStripeSignatureFromText };
